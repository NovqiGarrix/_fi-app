...
import type Income from "@/model/Income.model";
...
function IncomesComp({ incomes }: IncomesProps) {
  ...
  const { mutateAsync: deleteIncome, isPending: isDeletingIncome } =
    useMutation({
  ...
  return (
    <View>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <AnimatedTouchableOpacity
            onLongPress={() => setLongPressIncome(item)}
            entering={FadeInRight.duration(400).delay(index * 50)}
            className="p-7 mr-3 rounded-3xl min-w-[200px] bg-[#1E1E1E]"
          >
            ...
            <Text
              style={{ fontFamily: Fonts.ManropeBold }}
              className="text-white text-2xl mt-1.5"
            >
      ...
      <Modal
        transparent
        visible={!!longPressIncome}
        animationType="fade"
        onRequestClose={() => setLongPressIncome(null)}
      >
          ...
          <View className="w-full rounded-2xl bg-[#1c1c1c] py-6">
            <View className="flex justify-between px-6 pb-4 mb-2">
              ...
              >
                ...
                <Text className="text-dark-text italic font-bold">
                  {longPressIncome?.title} -{" "}
                  {formatMoney(longPressIncome?.amount as number)}
                </Text>
            ...
            <View className="w-full flex-row items-center justify-end gap-3 px-6">
              <TouchableOpacity
                ...
                onPress={() => setLongPressIncome(null)}
      ...
      {incomes.length <= 0 && (
        <View className="items-center justify-center">
          ...
          >
            You haven&apos;t added any income yet. Press the plus button above
            to add your first income.
          </Text>
        </View>
      )}
    </View>
